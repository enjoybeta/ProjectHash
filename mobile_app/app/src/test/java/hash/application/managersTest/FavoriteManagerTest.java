package hash.application.managersTest;

import org.junit.Test;

import java.util.ArrayList;

import hash.application.dataType.Recipe;
import hash.application.managers.FavoriteManager;

import static junit.framework.Assert.*;

public class FavoriteManagerTest {
    FavoriteManager manager = FavoriteManager.INSTANCE;
    // TODO Cannot be done by local environment
    @Test
    public void addRecipeTest() {
//        ArrayList<String> ingredients = new ArrayList<String>();
//        ingredients.add("Ingredient1");
//        ingredients.add("Ingredient2");
//        ingredients.add("Ingredient3");
//        Recipe a = new Recipe("Soup", "1", 2000, "example.com", 4, "NULL", "example.com", ingredients);
//        ingredients = new ArrayList<String>();
//        ingredients.add("Ingredienta");
//        ingredients.add("Ingredientb");
//        ingredients.add("Ingredientc");
//        Recipe b = new Recipe("Rice", "2", 3000, "example.com", 4, "NULL", "example.com", ingredients);
//        manager.addRecipe(a);
//        manager.addRecipe(b);
//        assertTrue(manager.findRecipeById("1"));
//        assertTrue(manager.findRecipeById("2"));
//        assertFalse(manager.findRecipeById("3"));
//        assertTrue(manager.findRecipeByName("Soup"));
//        assertTrue(manager.findRecipeByName("Rice"));
//        assertFalse(manager.findRecipeByName("Test"));
    }
}
